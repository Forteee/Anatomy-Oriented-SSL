#ref:https://github.com/TripleCoenzyme/ResNet50-Unet/blob/master/model.py
import torch
...
class Resnet_Unet(nn.Module):
    ...
    def __init__(self, BN_enable=True, resnet_pretrain=False):
        ...
        self.pool1_label = torch.nn.MaxPool2d(28, stride=None, padding=0, dilation=1, return_indices=False,
                                              ceil_mode=False)
        if self.BN_enable:
    ...
    def forward(self, x):
        x = self.firstconv(x) #10,1,224,224
        x = self.firstbn(x) #10,64,112,112
        x = self.firstrelu(x)
        x_ = self.firstmaxpool(x) #10,64,56,56

        e1 = self.encoder1(x_)#10,256,56,56
        e2 = self.encoder2(e1)#10,512,28,28
        e3 = self.encoder3(e2)#10,1024,14,14

        center = self.center(e3)#10,1024,28,28

        d2 = self.decoder1(torch.cat([center, e2], dim=1))#10,512,56,56
        d3 = self.decoder2(torch.cat([d2, e1], dim=1))#10,256,112,112
        d4 = self.decoder3(torch.cat([d3, x], dim=1)) #10,64,224,224

        # #回归任务
        feature_x = self.pool1_label(center)  #
        # label_slice_pre = self.linear_(feature_x.view(feature_x.shape[0], -1))
        label_slice_pre = self.global_layers(feature_x.view(feature_x.shape[0],-1))#10,1

        return self.final(d4),label_slice_pre  #10,2,224,224