...
class ResNet(nn.Module):
    def __init__(self, block, layers, dilation=[1, 1, 1, 1], bn_momentum=0.0003, is_fpn=False,pretrained=True):
        self.inplanes = 128
        self.is_fpn = is_fpn
        super(ResNet, self).__init__()
        self.conv = nn.Conv2d(1, 64, 3, stride=2, padding=1, bias=False)
        ...
        self.relu = nn.ReLU(inplace=False)
        self.layer = self._make_layer(block, 64, layers[0], stride=1, dilation=dilation[0], bn_momentum=bn_momentum)
    ...
    def forward(self, x, start_module=1, end_module=5):
        if start_module <= 1:
            x = self.relu1(self.bn1(self.conv(x)))
        ...
        x =self.layer(x)
        features.append(x)
        for i in range(start_module+1, end_module + 1):
