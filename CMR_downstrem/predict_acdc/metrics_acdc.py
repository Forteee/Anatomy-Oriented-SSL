...
def compute_metrics_on_directories_raw(dir_gt, dir_pred):
    ...
    for p_gt, p_pred in zip(filenames_gt, filenames_pred):
        ...
        gt, _, header = util.load_nii(p_gt)
        pred, _, _ = util.load_nii(p_pred)
        ...
        for struc in [3,1,2]:
            ...
            if np.sum(gt_binary) == 0 and np.sum(pred_binary) == 0:
                dices_list.append(1.0)
                assd_list.append(0.0)
                hausdorff_list.append(0.0)
                dice_loc = []
                for i in range(gt_binary.shape[2]):
                    dice_loc.append(0.0)
                dices_slices.append(dice_loc)
                ...
                dices_list.append(0.0)
                assd_list.append(1.0)
                hausdorff_list.append(1.0)
                dice_loc = []
                for i in range(gt_binary.shape[2]):
                    dice_loc.append(0.0)
                dices_slices.append(dice_loc)
                ...
                dice_loc = []
                for i in range(gt_binary.shape[2]):
                    dice_loc.append(dc(gt_binary[:, :, i], pred_binary[:, :, i]))
                dices_slices.append(dice_loc)
    ...
    df = pd.DataFrame({'dice': dices_list, 'hd': hausdorff_list, 'assd': assd_list,
                       'vol': vol_list, 'vol_err': vol_err_list, "location": dices_slices,
                       'phase': cardiac_phase, 'struc': structure_names, 'filename': file_names})

    return df


def compute_metrics_on_patients_raw(dir_gt, dir_pred):
    filenames_gt = sorted(glob(os.path.join(dir_gt, '*')), key=natural_order)
    filenames_pred = sorted(glob(os.path.join(dir_pred, '*')), key=natural_order)
    ...
    for p_gt, p_pred in zip(filenames_gt, filenames_pred):
        ...
        for struc in [3,1,2]:
            ...
            for i in range(gt_binary.shape[2]):
                ...
                if np.sum(gt_binary[:, :, i]) == 0 and np.sum( pred_binary[:, :, i]) == 0:
                    dice_loc.append(1.0)
                    assd_loc.append(0.0)
                    hd_loc.append(0.0)

                elif np.sum(pred_binary[:, :, i]) > 0 and np.sum(gt_binary[:, :, i]) == 0 or np.sum(pred_binary[:, :, i]) == 0 and np.sum(gt_binary[:, :, i]) > 0:
                    logging.warning(  'Structure missing in either GT (x)or prediction. ASSD and HD will not be accurate.')
                    dice_loc.append(0.0)
                    assd_loc.append(1.0)
                    hd_loc.append(1.0)
                    ...
                    dice_loc.append(dc(gt_binary[:, :, i], pred_binary[:, :, i]))
                    assd_loc.append(assd(gt_binary[:, :, i], pred_binary[:, :, i]))
                    hd_loc.append(hd(gt_binary[:, :, i], pred_binary[:, :, i]))

            hausdorff_list.append(assd_loc)
            assd_list.append(hd_loc)
            dices_list.append(dice_loc)
...
def print_latex_tables(df, eval_dir):
    ...
    with open(out_file, "w") as text_file:
        ...
        text_file.write('\n\n-------------------------------------------------------------------------------------\n')
        text_file.write( 'LV dice & {:.6f}\,({:.6f})\n '.format(np.mean(df.loc[df['struc'] == 'LV']['dice']), np.std(df.loc[df['struc'] == 'LV']['dice'])))
        text_file.write('RV dice & {:.6f}\,({:.6f}) \n'.format(np.mean(df.loc[df['struc'] == 'RV']['dice']),
                                                              np.std(df.loc[df['struc'] == 'RV']['dice'])))
        text_file.write('Myo dice & {:.6f}\,({:.6f})\n '.format(np.mean(df.loc[df['struc'] == 'Myo']['dice']),
                                                               np.std(df.loc[df['struc'] == 'Myo']['dice'])))
        text_file.write('Mean dice & {:.6f}\,({:.6f})\n '.format(np.mean(df['dice']),
                                                                np.std(df['dice'])))
        text_file.write('\n\n-------------------------------------------------------------------------------------\n')
        text_file.write('LV assd & {:.6f}\,({:.6f})\n '.format(np.mean(df.loc[df['struc'] == 'LV']['assd']),
                                                               np.std(df.loc[df['struc'] == 'LV']['assd'])))
        text_file.write('RV assd & {:.6f}\,({:.6f}) \n'.format(np.mean(df.loc[df['struc'] == 'RV']['assd']),
                                                               np.std(df.loc[df['struc'] == 'RV']['assd'])))
        text_file.write('Myo assd & {:.6f}\,({:.6f})\n '.format(np.mean(df.loc[df['struc'] == 'Myo']['assd']),
                                                                np.std(df.loc[df['struc'] == 'Myo']['assd'])))
        text_file.write('Mean assd & {:.6f}\,({:.6f})\n '.format(np.mean(df['assd']),
                                                                 np.std(df['assd'])))
        text_file.write('\n\n-------------------------------------------------------------------------------------\n')
...
def print_stats1(df, eval_dir):

    out_file = os.path.join(eval_dir, 'summary_report1.txt')
    ...
    with open(out_file, "w") as text_file:
        ...
        for struc_name in ['LV', 'RV', 'Myo']:
            ...
            for cardiac_phase in ['ED', 'ES']:
                ...
                for loc in range(6):
                    slice_dice[loc] = []
                for index in range(len(dat["location"])):
                    person=dat["location"].iloc[index]
                    for loc in range(6):
                        slice_dice[loc].append(person[loc])

                for loc in range(6):
                    text_file.write('{}, location dice: {},std:{} \n'.format(loc,  np.mean(slice_dice[loc]),np.std(slice_dice[loc])))
...
def print_stats2(df, eval_dir,path_pred):

    out_file = os.path.join(eval_dir, 'patients_report1.txt')
    ...
    with open(out_file, "w") as text_file:
        ...
        filenames = sorted(glob(os.path.join(path_pred, '*')), key=natural_order)
        for p_gt in filenames:
            ...
            for struc_name in ['LV', 'RV', 'Myo']:
                text_file.write('    {}\n'.format(struc_name))
                text_file.write('\n')
                cardiac_phase=filename.split('.nii.gz')[0].split('_')[-1]
                # for cardiac_phase in ['ED', 'ES']:

                text_file.write('       {}\n'.format(cardiac_phase))
                dat = df.loc[(df['phase'] == cardiac_phase) & (df['struc'] == struc_name)&(df['filename'] == filename)]
                for measure_name in ['dice', 'hd', 'assd']:
                    dat[measure_name].index=[0]
                    text_file.write('       {} -- mean (std): {:.3f} ({:.3f}) \n'.format(measure_name, np.mean(dat[measure_name][0]), np.std(dat[measure_name][0])))
                    text_file.write('\n')
                    for i in range(len(dat[measure_name][0])):
                        text_file.write('       slice {} : {:.3f}  \n'.format(i,dat[measure_name][0][i]))
...
def main(path_gt, path_pred, eval_dir):
    ...
    if os.path.isdir(path_gt) and os.path.isdir(path_pred):
        ...
        df1 = compute_metrics_on_directories_raw(path_gt, path_pred)
        df2 = compute_metrics_on_patients_raw(path_gt, path_pred)
        print_stats1(df1, eval_dir)
        print_stats2(df2, eval_dir, path_pred)
        print_latex_tables(df1, eval_dir)
        boxplot_metrics(df1, eval_dir)

        logging.info('------------Average Dice Figures----------')
        logging.info('Dice 1 (LV): %f' % np.mean(df1.loc[df1['struc'] == 'LV']['dice']))
        logging.info('Dice 2 (RC): %f' % np.mean(df1.loc[df1['struc'] == 'RV']['dice']))
        logging.info('Dice 3 (Myo): %f' % np.mean(df1.loc[df1['struc'] == 'Myo']['dice']))
