...
class UNet(nn.Module):
    ...
    def __init__(self, n_channels=1, n_classes=2, feature_scale=1, is_deconv=True, is_batchnorm=True):
        ...
        self.linear1_ = nn.Linear(1024, 1)
    ...
    def forward(self, inputs):
        ...
        final = self.final_(up1) 

        feature_x = self.pool1_label(center)  
        label_slice_pre = self.linear1_(feature_x.view(feature_x.shape[0],-1))

        return final, label_slice_pre

class Encoder(nn.Module):
    def __init__(self,n_channels=1, is_deconv=True, is_batchnorm=True):
...
class Decoder(nn.Module):
    def __init__(self, n_classes_heat=2, is_deconv=True, is_batchnorm=True):
